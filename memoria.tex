...
\begin{itemize}
	...
	aprender el funcionamiento de A-Frame \ref{sec:aframe} o aprender lenguajes de programación como JavaScript \ref{sec:javascript}.
...
La gran base de este proyecto es A-Frame\cite{aframe}, un framework cimentado sobre JavaScript que ofrece la oportunidad de crear escenas en VR (Virtual Reality o Realidad Virtual) sin la necesidad de instalar ninguna dependencia. Gracias a la unión de HTML, JavaScript, WebGL, WebXR y Three.js (comentados en sus respectivos apartados más adelante), A-Frame lleva la arquitectura ECS (Entity-Component-System o Entidad-Componente-Sistema) a otro nivel. Con esta arquitectura, se puede enlazar componentes a entidades, actualizarlos e incluso eliminarlos. Al obtener el DOM característico de HTML, es posible acceder a las entidades con sentencias simples (Query Selectors).
...
Como se puede observar, dentro del campo "head" del código, se añaden todas las dependencias necesarias. En este caso, al ser un ejemplo sencillo, es necesario únicamente el script básico de A-Frame. En el elemento "body" se inserta la escena (que agrupará todos los elementos que posteriormente se quieran añadir). En él, se encuentran un total de cinco entidades: un cubo, una esfera, un cilindro, un plano y el cielo, respectivamente. Todas ellas contienen unos atributos únicos que definen su tamaño, color o posición.
...
\begin{figure}
	...
	\caption{Escena de ejemplo Introducción a A-Frame}
	\label{fig:aframe}
\end{figure} 
...
Para finalizar, resta detallar el papel del DOM (Document Object Model) en un documento HTML. Consiste en una interfaz que proporciona una estructura lógica de todos los elementos implicados en el documento y los organiza en nodos, apodando a esta unión árbol de nodos (Figura \ref{fig:dom}). Esto crea una potente unión entre las entidades mencionadas antes en A-Frame y lo que se comentará a continuación en el apartado de JavaScript, gracias al DOM se podrá acceder a ellas y manejarlas al antojo del programador.
...
Dicho esto, igual que se accede al título, se puede acceder a un componente de la escena en A-Frame: cubos, cilindros, el cielo o incluso a la cámara de nuestra escena. Aporta infinidad de opciones: crear botones que activen eventos, movimiento a distintas entidades, etc. Junto con todo lo que ofrece A-Frame, JavaScript será la base de este proyecto.
...
El objetivo de esta fase es aprender a usar A-Frame, en esto se incluye: aprender que son las entidades, crear entidades, crear eventos, crear componentes, dinámicas y físicas, etc. Como JavaScript es aún desconocido, otro gran objetivo es ir aprendiendo a la vez que se enlaza con A-Frame. El fin del minijuego es hacer un pequeño recorrido en el que se tengan que realizar distintas acciones para llegar a la meta.
...
\begin{enumerate}
	...
	\item Ya subido en la plataforma puedes girar 180 grados para encontrarte un nuevo obstáculo en movimiento, la piedra gigante (Figura \ref{fig:mini_bloque}). Pero para llegar a ella primero se deben empujar las dos piedras que cortan el camino, haciendo así que caigan al vacío.
	\begin{figure}
		\centering
...
Para crear este pequeño juego se comienza con un archivo HTML en el que se importan los scripts necesarios de A-Frame para poder comenzar a crear entidades, estos son algunos de ellos:
...
Como iniciación a las nuevas tecnologías, el resultado fue notablemente bueno. Se pudo practicar con muchas de las posibilidades que ofrecía A-Frame mientras se aprendía JavaScript. Además, se tuvo el primer contacto con las gafas de realidad virtual en las que se pudo depurar el final del juego (Figura \ref{fig:mini_fin}). El minijuego se encuentra alojado en la página de GitHub del proyecto y puede ser jugado\footnote{https://javierbravodonaire.github.io/A-frame/Sprint\%200/my-examples/minigame.html}. El código JavaScript\footnote{https://github.com/JavierBravoDonaire/A-frame/blob/master/Sprint\%200/my-examples/minijuego.js} se encuentra a su vez alojado en la carpeta correspondiente a este "Sprint" junto a su HTML\footnote{https://github.com/JavierBravoDonaire/A-frame/blob/master/Sprint\%200/my-examples/minigame.html}. Para terminar, se mantuvo una reunión en la que se comentó lo aprendido gracias a este "Sprint" y dió comienzo al siguiente apartado.
...
En esta primera tarea se trabajó en una escena que ofreciese un panel con distintos botones, que permitiese la elección de diferentes figuras. Para empezar, se debe crear un entorno en el que el usuario se sienta cómodo, y para ello, se crea un plano en horizontal (el cual estará dotado de una rotación de 90 grados) que simule el suelo y se dota a la escena del elemento "sky" (con una textura que se asemeje al cielo) para que parezca que nos encontramos en un plano real. Una vez establecido el suelo, ya se pueden empezar a situar nuestros elementos.

El panel será entonces un plano, de un tamaño razonable, en el cual se colocarán los botones. Para crear los botones, se usará un cubo al que le modifica su propiedad "depth" (profundidad), para dar la sensación de apretar un botón (Figura \ref{fig:Rooster_TFG}). Cada botón tendrá un texto donde se indica qué objeto crea. Este texto se añade creando una entidad "a-text" y enlazándola con el botón.
...
Por último, se añadirá una función para que, al igual que se puede elegir el estilo de las figuras, sea posible elegir su tamaño.
...
Una vez se tienen las figuras con su esqueleto y sus ejes, resta generar el eje de la escena, que es el que marca los tamaños y posiciones. Para ello, la idea es generar en el plano 'xz' una cuadrícula de tamaño 1, que permita observar la profundidad y desplazamiento, y un plano 'y' que tratará de un eje con marcas cada distancia 1 (estas marcas serán líneas más pequeñas que simulan un eje como el que podemos ver en una gráfica).

Para esta tarea, se crea una función que genera líneas, en los distintos planos hasta una distancia que se le pasa como parámetro, por si se quiere hacer más grande o más pequeño, creando así la cuadrícula y el eje 'y'. Además, para que este modo se diferencie y deje apreciar bien los ejes, se cambia la textura del suelo por un color verde que resalte el negro de los ejes.
