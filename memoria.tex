...
El proyecto realizado se centra en la creación de una interfaz para la construcción de escenas en realidad virtual, cuya máxima consiste en acercar a usuarios que no son expertos en este ámbito al manejo de entornos virtuales. Este trabajo ha sido creado gracias al framework de A-Frame para producir experiencias en realidad virtual, enlazado con distintas tecnologías, como JavaScript y HTML.


Fusionando los elementos HTML con el lenguaje JavaScript, podemos manejar distintos eventos y crear funcionalidad, para añadir dinamismo a la escena. A-Frame, el cual está construido sobre Three.js, se encargará de generar el entorno, y agregando la biblioteca WebGL se renderizarán todos los elementos.

La escena es accesible desde un navegador como con un dispositivo  de realidad virtual, gracias a WebVR, permitiendo obtener una experiencia completa.
...
The current project's aim is the creation of an interface for building scenes in virtual reality, which ultimate goal consist on bring amateur users closer to this technology. This idea has been created thanks to the A-Frame framework linked with several technologies, such as HTML or JavaScript.

Fusing HTML elements with JavaScript, we can handle different events and add functionalities, with the aim of providing dynamism. Furthermore, A-Frame is built on Three.js. It will generate the environment, rendering all the objects through WebGL.

The scene is accessible from a browser or a virtual reality device, allowing to obtain an immersive experience, using WebVR API.
...
A-Frame no proporciona ninguna entidad que represente una barra deslizante, pero gracias a GitHub podemos encontrar muchos scripts que definen varios componentes que nos interesara importar directamente a nuestro proyecto, entre ellos la barra deslizable. En este caso, la biblioteca aportada por el usuario \textbf{rdub80}\footnote{https://github.com/rdub80/aframe-gui}, proporcionaba diversos elementos como barras de progreso, etiquetas y la empleada, barra deslizante.
...
Esta iteración se corresponde con la recta final del proyecto, y como todas las anteriores, comienza con una reunión con Jesús. En ella se establecieron los objetivos que se exponen en el siguiente apartado y se inició la redacción de este escrito. 

\subsection{Objetivo}

Es hora de adaptar el proyecto a su uso mediante unas gafas de realidad virtual, lo que implica dividir este en dos ramas, la versión de escritorio (navegador) y la versión con gafas de realidad virtual. Cada una tendrá su propio modo de uso y componentes. Para finalizar, se agrega una última funcionalidad, que consta en un modo de edición, donde se generan los ejes correspondientes a la escena y a las entidades para permitir una visión más clara de los tamaños y posiciones.
...
\begin{itemize}
	\item desktop.html\footnote{https://github.com/JavierBravoDonaire/A-frame/blob/master/Sprint\%204/desktop.html}
	\item glasses.html\footnote{https://github.com/JavierBravoDonaire/A-frame/blob/master/Sprint\%204/glasses.html}
\end{itemize}
...
Para dotar de ejes a las figuras, debemos agregar como nodos hijos tres entidades "line" (línea). Al ser hijas de la entidad, se moverán con ella y su posición central (0,0,0) será igual a la de la entidad o no a la de la escena, lo que nos permite generar los ejes modificando solo un valor de posición de cada línea (eje 'x', eje 'y' y eje 'z'). La entidad linea contiene las propiedades inicio y fin, si queremos representar el eje 'x' entonces debemos crear una línea como sigue:

\begin{verbatim}
<a-entity line="start: 0, 0, 0; end: 2 0 0; color: black"</a-entity>
\end{verbatim}

Una vez tenemos las figuras con su esqueleto y sus ejes, resta generar el eje de la escena, que es el que marca los tamaños y posiciones. Para ello, la idea es generar en el plano 'xz' una cuadrícula de tamaño 1, que nos permita observar la profundidad y desplazamiento, y un plano 'y' que tratará de un eje con marcas cada distancia 1 (estas marcas serán lineas más pequeñas que simulan un eje como el que podemos ver en una gráfica).

Para esta tarea, se crea una función que genera lineas en los distintos planos hasta una distancia que se le pasa como parámetro, por si queremos hacerlo más grande o pequeño, creando así la cuadrícula y el eje 'y'. Además, para que este modo se diferencie y deje apreciar bien los ejes, se cambia la textura del suelo por un color verde que resalte el negro de los ejes.
...
Como las dos manos son iguales pero cambiando un atributo, es más factible crear un mixin que las represente, en el que añadamos los atributos generales. Entre estas propiedades nos encontramos "raycaster" y controles varios (vive-controls, oculus-touch-controls,
...
Una vez creado los mixin, solo necesitamos crear las dos manos como hijas del usuario en una posición que las simulen correctamente. En este caso, decidí emplear la función láser de los mandos, lo que no representa unas manos en la escena, sino que vemos los mandos con un rayo láser con el que podemos apuntar y seleccionar todos los elementos. Esta decisión se tomó para garantizar una movilidad mayor, fusionando los controles del usuario con los "joysticks" de los mandos y el apuntado con láser.
...
\begin{itemize}
	...
	\item Como el uso del teclado mientras usamos las gafas y los mandos no es muy intuitivo, se añaden en el panel inicial dos botones para entrar y salir del modo edición, Figura \ref{fig:vr}.
\end{itemize}
...
Se consigue así la versión final de este proyecto, logrando todas las funcionalidades propuestas con éxito. El código HTML de las dos versiones se adjunto anteriormente, pero el código JavaScript\footnote{https://github.com/JavierBravoDonaire/A-frame/blob/master/Sprint\%204/improve.js} se adjunta en este apartado. Tanto la versión de escritorio como la de las gafas, usan el mismo JavaScript con el objetivo de lograr una unificación de ambas, en caso de que sea necesario. Por último, la versión final del proyecto se encuentra alojada en el repositorio de GitHub para su libre uso, en versión de escritorio\footnote{https://javierbravodonaire.github.io/A-frame/Sprint\%204/desktop.html} y versión VR\footnote{https://javierbravodonaire.github.io/A-frame/Sprint\%204/glasses.html}.
...
\begin{itemize}
	\item \textbf{Modo escritorio}. Para mover la cámara debemos pulsar con el botón y arrastrar hacia la dirección deseada. Para moverse por la escena se pueden usar tanto las flechas de dirección como las teclas "WASD". Para interactuar con los distintos elementos de la escena se usa el ratón, haciendo click sobre estos.
	\item \textbf{Gafas VR}.  Con los mandos de las gafas nos movemos y el movimiento viene marcado por la dirección en la que miramos. Existe la posibilidad de movernos en la escena moviéndonos nosotros. Aparecerán dos mandos en la escena con un láser que funcionarán como el ratón e interactuaremos con el apuntando a los elementos.
\end{itemize}

La escena comienza con un podio y un panel en el que podemos elegir el método de creación de entidades: mesa o panel. Son dos botones por lo que, siguiendo las instrucciones anteriores, interactuaremos con ellos. Pulsando sobre las entidades de la mesa o los botones del panel se editará la entidad que aparece en el podio. Además, se puede editar la textura, color y tamaño de la figura mediante los botones y la barra deslizante. Se pueden combinar colores y texturas, esto es, eligiendo una textura y después un color para colorear esta del color indicado. El botón que representa el color blanco servirá para reiniciar la figura.
...
En ella se declaran aquellos elementos que necesitamos que aparezcan en el inicio y aquellos que no van a cambiar (Suelo, cielo o panel principal, por ejemplo). El código que hemos visto es una versión simplificada con los componentes, obviando datos como posiciones y texturas para desarrollar este apartado de una forma más técnica. Dentro de la sección "assets" se encuentran todas las imágenes, modelos 3D y mixin empleados.

Como se observa, varios elementos tienes nodos hijos: panel, podium y user camera. El panel contendrá los botones para generar la mesa y el otro panel, el usuario contiene la cámara y el podio mantiene la entidad. Cuando la entidad abandone el podio, esta pasará a ser un elemento de la escena como podría ser el cielo, y el clon la sustituirá modificando de esta forma el DOM.

\subsubsection{Componentes:}

\begin{itemize}
	\item \textit{Editable}. Este componente se encarga de la generación de la paleta de colores y texturas cuando una figura deja el podio. En él, se configuran los botones (posiciones, funciones y tamaños) y por último, añade la paleta como hija de la figura en cuestión. Este componente se encuentra en la escena iniciada en la entidad del podio, para conseguir asi que la paleta se genere inmediatamente.
	\item \textit{Button}. Funciona de forma conjunta al anterior, ya que cada botón de la paleta recibirá este componente como atributo (esto se hace desde el anterior componente), y será este el que se encargue de modificar la textura o el color de la entidad padre. Este no se encontrará en la escena inicial, ya que hasta que no se cree la paleta no se crearán los botones, que son los que adoptan este componente.
	\item \textit{Create-table}. Es el encargado de crear la mesa de entidades y posicionar las figuras encima de esta (además de añadir sus parámetros y animaciones). Se encuentra enlazado en el botón de creación de mesa al iniciar la escena.
	\item \textit{Create-panel}. Al igual que se crea la mesa, se crea el panel de colores y la barra deslizante a través de este componente. Tiene la misma funcionalidad que el anterior y se encuentra también al inicio.
	\item \textit{Edit-mode}. Este componente se enlaza con el elemento que representa al usuario y es el encargado de detectar si se pulsa la tecla necesaria para entrar y salir del modo edición. Dentro de este están declarados todos los parámetros y funciones para dotar tanto a la escena como a las entidades de ejes.
	\item \textit{Enter-edit-mode}. Como pulsar teclas no es intuitivo, se crean dos botones para entrar y salir del modo edición cuando se usen las gafas VR. Este componente y el siguiente se encargan de ello. Al igual que el anterior, se encarga de los ejes y de detectar en este caso solo el botón de entrada.
...
Por último, cabe mencionar aquellos componentes usados que nos aporta A-Frame y se han usado en la escena:

\begin{itemize}
	\item \textit{Clickable}. Este componente nos aporta el evento de click que se activa al pulsar sobre las entidades que lo poseen. Junto con el siguiente componte, nos permite manejar la gran parte de los eventos que suceden en la escena (botones).
	\item \textit{Grabbable}. Dota a las entidades de la capacidad de agarrarlas y moverlas por toda las escena. 
	\item \textit{Stretchable}. Gracias a este componente podemos realizar el gesto con los mandos para modificar el tamaño de las figuras en nuestra escena.
\end{itemize}
...
En este apartado, echaremos la vista atrás al capítulo de objetivos (Capítulo \ref{sec:intro}) y se debatirá si se logró la consecución y los problemas que surgieron.
